...
type AdminTab = "dashboard" | "export-logs";
      ...
      <div className="flex min-h-screen">
        ...
        <section className="min-w-0 flex-1">
          ...
          <div className="mx-auto w-full max-w-[1240px] px-4 py-6 sm:px-6 lg:px-8 lg:py-8">
            ...
            {activeTab === "dashboard" ? <DashboardPanel /> : <ExportLogsPanel />}
          </div>
        </section>
      </div>
    </main>
    ...
    <>
      ...
      >
        ...
        <nav className="mt-8 space-y-2">
          {navItems.map((item) => (
            <SidebarButton key={item.id} item={item} isActive={activeTab === item.id} onClick={() => onSelect(item.id)} />
          ))}
        </nav>
...
function ExportLogsPanel() {
  return (
    <section className="rounded-2xl border border-white/10 bg-[#101827]/90 p-5 shadow-panel">
      <div className="flex flex-col gap-5 md:flex-row md:items-center md:justify-between">
        <div className="flex items-start gap-3">
          ...
          <div>
            <h3 className="text-xl font-extrabold tracking-normal text-white">Export Logs</h3>
            <p className="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">
              Export controls will be added here for lookup audit records, cache events, and administrative activity.
            </p>
          </div>
        </div>
